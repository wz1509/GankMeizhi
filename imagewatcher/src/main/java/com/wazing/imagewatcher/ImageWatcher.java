package com.wazing.imagewatcher;
...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    private int mErrorImageRes = R.drawable.image_error_24dp;
     ...
     *
     * @param position       被点击的ImageView position位置
    ...
    public void show(int position, List<ImageView> imageGroupList, final List<String> urlList) {
        ...
        if (imageGroupList == null || urlList == null || imageGroupList.size() < 1 ||
                urlList.size() < imageGroupList.size()) {
            String info = "#imageGroupList " + (imageGroupList == null ? "null" : "size : " + imageGroupList.size());
        ...
        if (imageGroupList.get(position).getDrawable() == null) return;
